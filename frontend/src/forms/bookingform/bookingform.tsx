...
const BookingForm = ({ currentUser, paymentIntent }: Props) => {
  ...
  const { mutate: bookRoom,isLoading } = useMutation(apiClient.createRoomBooking, {
    ...
    onError: () => {
      showToast({ message: "Booking Error saving Booking", type: "ERROR" });
    },
  });
  ...
  return (
    <form className="grid grid-cols-1 gap-5 rounded-lg border border-slate-300 p-5" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="flex justify-end">
        <button 
        disabled={isLoading}
        type="submit" className="bg-blue-600 text-white p-2 font-bold hover:bg-blue-500 text-md disables:bg-gray-500">
